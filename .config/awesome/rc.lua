...
local terminal = "open-terminal-in-cwd.sh"
...
local taglist_mouse_bindings = gears.table.join(
...
local tasklist_mouse_bindings = gears.table.join(
...
-- Change current focussed client by direction. If there are
-- no clients to focus in the specified direction, cycle
-- clients in clockwise or anticlockwise direction.
-- @param dir Direction like "up", "left", etc.
-- @param cycle Either "clockwise" or "anticlockwise"
local function focusto(dir, cycle)
  return function()
    local before = client.focus
    awful.client.focus.bydirection(dir)
    local after = client.focus
    if before == after then -- no client to focus in `dir` direction
      local clockwise = cycle == "clockwise"
      awful.client.cycle(clockwise)
    end
  end
end
...
local globalkeys = gears.table.join(
  ...
  mod("j", focusto("down", "clockwise")),
  mod("k", focusto("up", "anticlockwise")),
  mod("h", focusto("left", "anticlockwise")),
  mod("l", focusto("right", "clockwise")),
